...
angular.module('petShopApp', ['angularPayments', 'mm.foundation', 'ngAnimate', 'angularSpinner'])
	...
	.controller('PetShopCtrl', function ($scope, $http, $modal) {
		...
		$http.get('mobiles.json').success(function (response) {
			$scope.mobiles = response.mobiles;
		});
		...
		$scope.addToCart = function (mobile) {
			...
			if (!found) {
				$scope.cart.push(angular.extend({quantity: 1}, mobile));
			}
		};

		$scope.getCartPrice = function () {
			var total = 0;
			$scope.cart.forEach(function (mobile) {
				total += mobile.price * mobile.quantity;
			});
			return total;
		};
